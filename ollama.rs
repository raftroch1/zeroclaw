...
impl Provider for OllamaProvider {
    ...
    /// Unified chat entry point that converts ToolSpec to OpenAI-compatible JSON.
    ///
    /// This override is REQUIRED because the default Provider::chat() trait implementation
    /// does not convert ToolSpec to JSON for providers that support native tools.
    /// Without this, tools are never passed to the Ollama API, breaking tool invocation.
    async fn chat(
        &self,
        ...
        // Convert ToolSpec to OpenAI/Ollama-compatible JSON and delegate to chat_with_tools.
